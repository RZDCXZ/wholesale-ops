    ...
    <form action={formAction} className="grid gap-5" onChange={() => setDirty(true)}>
      ...
      <section className="grid gap-5 rounded-lg border border-[#e4e7ec] bg-white p-5 sm:grid-cols-2">
        <Field label="客户编码" name="customerCode" errors={state.fieldErrors?.customerCode} hint={customer ? "客户编码创建后不可修改。" : "创建后不可修改，例如 KH-0009。"}>
          <Input aria-label="客户编码" name="customerCode" defaultValue={customer?.customerCode} disabled={Boolean(customer)} aria-invalid={invalid("customerCode")} aria-describedby={describedBy("customerCode")} className="font-mono" />
        </Field>
        <Field label="客户名称" name="name" errors={state.fieldErrors?.name} hint="客户名称允许重复，以客户编码稳定识别。">
          <Input aria-label="客户名称" name="name" defaultValue={customer?.name} aria-invalid={invalid("name")} aria-describedby={describedBy("name")} />
        </Field>
        <Field label="联系人" name="contactName" errors={state.fieldErrors?.contactName}>
          <Input aria-label="联系人" name="contactName" defaultValue={customer?.contactName} aria-invalid={invalid("contactName")} aria-describedby={describedBy("contactName")} />
        </Field>
        <Field label="电话" name="phone" errors={state.fieldErrors?.phone}>
          <Input aria-label="电话" name="phone" type="tel" defaultValue={customer?.phone} aria-invalid={invalid("phone")} aria-describedby={describedBy("phone")} />
        </Field>
        <Field label="客户负责人" name="responsibleSalesId" errors={state.fieldErrors?.responsibleSalesId} hint={customer ? "负责人调整使用详情页的专门操作。" : currentSales ? "销售创建客户时，负责人固定为自己。" : "只能选择启用的销售账号。"}>
          {mode === "create" && !currentSales ? (
            <FormSelect aria-label="客户负责人" name="responsibleSalesId" defaultValue="" aria-invalid={invalid("responsibleSalesId")} aria-describedby={describedBy("responsibleSalesId")} onValueChange={() => setDirty(true)} options={[{ value: "", label: "请选择客户负责人", disabled: true }, ...responsibleSalesOptions.map((option) => ({ value: option.id, label: option.name }))]} />
          ) : (
            <><Input aria-label="客户负责人" value={assignedSales?.name ?? ""} readOnly disabled /><input type="hidden" name="responsibleSalesId" value={assignedSales?.id ?? ""} /></>
          )}
        </Field>
        <Field label="默认账期" name="paymentTermDays" errors={state.fieldErrors?.paymentTermDays} hint="现结表示交付当天到期；账期天数从交付日开始计算。">
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2">
            <FormSelect aria-label="默认账期类型" name="paymentTermType" value={paymentTermType} onValueChange={(value) => { setPaymentTermType(value); setDirty(true); }} options={[{ value: "cash", label: "现结" }, { value: "credit", label: "指定天数" }]} />
            <div className="relative"><Input aria-label="默认账期天数" name="paymentTermDays" inputMode="numeric" defaultValue={customer?.paymentTermDays || 30} disabled={paymentTermType === "cash"} aria-invalid={invalid("paymentTermDays")} aria-describedby={describedBy("paymentTermDays")} className="pr-10" /><span className="pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-xs text-[#667085]">天</span></div>
          </div>
        </Field>
        <Field label="地址" name="address" errors={state.fieldErrors?.address} wide>
          <Textarea aria-label="地址" name="address" rows={3} defaultValue={customer?.address} aria-invalid={invalid("address")} aria-describedby={describedBy("address")} className="resize-y" />
        </Field>
        {mode === "create" ? (
          <label className="flex min-h-11 items-center gap-2 rounded-[7px] border border-[#d0d5dd] px-3 text-sm text-[#344054] sm:col-span-2"><Checkbox id="customer-enabled" name="enabled" defaultChecked />启用客户</label>
        ) : (
