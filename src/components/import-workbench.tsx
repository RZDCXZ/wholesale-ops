...
type ImportKind = "sku" | "customer" | "opening-inventory";
...
type OpeningInventoryPreviewRow = {
  rowNumber: number;
  skuId: string;
  skuCode: string;
  skuName: string;
...
const importConfigs = {
  ...
  "opening-inventory": {
    label: "期初库存",
    note: "建立库存流水起点",
    apiSegment: "opening-inventory",
    templateHref: "/api/imports/opening-inventory/template",
    inputLabel: "选择期初库存 Excel 文件",
    templateLabel: "下载期初库存模板",
    listHref: "/inventory",
    listLabel: "查看库存",
  },
} as const;
...
export type OpeningInventoryAvailability =
  | { allowed: true }
  | { allowed: false; code: string; message: string };

export function ImportWorkbench({
  openingInventoryAvailability,
}: {
  ...
  const errorRowCount =
    preview?.status === "invalid"
      ...
      : 0;
  const entityWithCount = (count: number) =>
    kind === "sku"
      ? `${count} 个 SKU`
      : kind === "customer"
        ? `${count} 个客户`
  ...
  return (
    <div className="grid gap-4">
      ...
      <section className="overflow-hidden rounded-lg border border-[#e4e7ec] bg-white">
        ...
        <div className="p-4">
          {phase === "idle" ? (
            kind === "opening-inventory" && !openingInventoryAvailability.allowed ? <Notice tone="danger" title="期初库存导入已停用" message={openingInventoryAvailability.message} /> : <div onDragOver={(event) => event.preventDefault()} onDrop={onDrop} className="grid min-h-52 place-items-center rounded-lg border-2 border-dashed border-[#b8c0cc] bg-[#fafbfc] p-6 text-center">
...
function ValidRowsTable({ kind, rows }: { kind: ImportKind; rows: PreviewRow[] }) {
  ...
  return <section><div className="mb-2 flex items-end justify-between gap-3"><div><h2 className="text-sm font-bold">可导入行</h2><p className="mt-1 text-xs text-[#667085]">共 {rows.length} 行</p></div></div>{kind === "sku" ? <SkuRowsTable rows={rows as SkuPreviewRow[]} /> : kind === "customer" ? <CustomerRowsTable rows={rows as CustomerPreviewRow[]} /> : <OpeningInventoryRowsTable rows={rows as OpeningInventoryPreviewRow[]} />}</section>;
}
...
function OpeningInventoryRowsTable({ rows }: { rows: OpeningInventoryPreviewRow[] }) {
  return <div className="overflow-x-auto rounded-lg border border-[#e4e7ec]"><table className="w-full min-w-[720px] border-collapse text-left text-[13px]"><thead className="bg-[#f8fafc] text-[#475467]"><tr>{["行号", "SKU 编码", "名称", "库存单位", "期初库存数量"].map((heading) => <th key={heading} className="border-b border-[#e4e7ec] px-3 py-2.5 font-semibold whitespace-nowrap">{heading}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.rowNumber} className="border-b border-[#eef0f3] last:border-b-0"><td className="px-3 py-2.5 tabular-nums">{row.rowNumber}</td><td className="px-3 py-2.5 font-mono text-xs font-semibold text-[#1d4ed8]">{row.skuCode}</td><td className="px-3 py-2.5 font-semibold">{row.skuName}</td><td className="px-3 py-2.5">{row.inventoryUnit}</td><td className="px-3 py-2.5 text-right font-semibold tabular-nums">{row.quantity}</td></tr>)}</tbody></table></div>;
}
