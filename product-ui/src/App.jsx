...
export function App() {
  ...
  return <AppShell page={page} setPage={setPage} role={role} setRole={setRole} mobileNav={mobileNav} setMobileNav={setMobileNav}>{content}
    ...
    {modal === "cancel" && <Modal title="取消销售单" danger actionLabel="取消并释放预占" onClose={() => setModal(null)} action={() => setModal(null)}><p>取消后将释放全部库存预占，现存量保持不变，销售单永久保留。</p><Field label="取消原因" required><textarea placeholder="请说明取消原因" /></Field></Modal>}
